...
class Matrixeval::Context::FindByCommandOptionsTest < MatrixevalTest
  ...
  def test_find_by_command_options
    context = Matrixeval::Context.find_by_command_options!({ruby: "3.1"})

    assert context.is_a?(Matrixeval::Context)
  ...
  def test_find_by_command_options_fail
    assert_raises "Can't find a corresponding matrix" do
      Matrixeval::Context.find_by_command_options!({ruby: "wrong"})
    end
  end
